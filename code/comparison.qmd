...
## Subset Marks of interest `A`

```{python}
adata_A = adata[adata.obs["marks"].astype(str) == "A"].copy()
```

Try smaller `min_hole_area_ratio` too. Only parameter to adjust.

```{python}
cc.gr.connected_components(adata_A, cluster_key='marks')
cc.tl.boundaries(adata_A, min_hole_area_ratio=0.001)
```
...
```
```{r}
sf_cellchart <- sf_to_binary_matrix(p0_poly, 0, 256, 0, 256, 256, 256) |>
  rotate_90_cw()

plot(p0_poly)

jaccard_binary(sf_cellchart, tissueImage)
